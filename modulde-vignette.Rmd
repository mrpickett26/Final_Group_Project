...
create and call at least 2 functions with documentation!! --> we have one for sd, but need another
...
wbg.pcov <- prcomp(wbg, cor=FALSE, scores=TRUE) #error with infinate or missing values
...
After creating the biplots in the previos step, we can see that area_worst and area_mean have much larger covariances than the other variables included. This is due to a scale issue and therefore indicates to us that we need to scale those variables for future PCA analysis. Let's calculate the eigenvectors and eigenvalues in the following steps.
